...
@media(max-width:900px){.gyc-grid{grid-template-columns:1fr}}
...
export default defineComponent({
  ...
  emits:['close','switch'],
  setup(props,{emit}){
    const cfg=reactive({enabled:false,permanently_delete:false,client_id:'',device_id:'',poll_interval:5,page_size:100,order_by:3,sort_type:1,access_token:'',refresh_token:'',logged_in:false});
    const loading=ref(false),saving=ref(false),message=ref(''),messageType=ref('');
    function apply(d={}){cfg.enabled=Boolean(d.enabled);cfg.permanently_delete=Boolean(d.permanently_delete);cfg.client_id=d.client_id||'';cfg.device_id=d.device_id||'';cfg.poll_interval=Number(d.poll_interval||5);cfg.page_size=Number(d.page_size||100);cfg.order_by=Number(d.order_by||3);cfg.sort_type=Number(d.sort_type??1);cfg.access_token=d.access_token||'';cfg.refresh_token=d.refresh_token||'';cfg.logged_in=Boolean(d.logged_in);}
    ...
    async function save(){saving.value=true;message.value='';try{const r=await postApi(props,'/config',{enabled:cfg.enabled,permanently_delete:cfg.permanently_delete,client_id:cfg.client_id,device_id:cfg.device_id,poll_interval:Number(cfg.poll_interval||5),page_size:Number(cfg.page_size||100),order_by:Number(cfg.order_by||3),sort_type:Number(cfg.sort_type??1),access_token:cfg.access_token,refresh_token:cfg.refresh_token});if(r?.success===false)throw new Error(r?.message||'保存失败');if(r?.data)apply(r.data);messageType.value='success';message.value=r?.message||'配置已保存';}catch(e){messageType.value='error';message.value=`保存失败：${e?.message||e}`;}finally{saving.value=false;}}
    onMounted(load);
    ...
    return()=>h('div',{class:'gy-config'},[h('style',css),h('div',{class:'gyc-shell'},[
      h('header',{class:'gyc-head'},[h('div',{class:'gyc-brand'},[h('div',{class:'gyc-logo'},'⚙'),h('div',[h('div',{class:'gyc-title'},'光鸭云盘助手 · 设置'),h('div',{class:'gyc-sub'},'运行参数与授权会话管理')])]),h('div',{class:'gyc-actions'},[h('button',{class:'gyc-btn',onClick:()=>emit('switch')},'状态页'),h('button',{class:'gyc-btn primary',disabled:saving.value,onClick:save},saving.value?'保存中…':'保存'),h('button',{class:'gyc-btn',onClick:()=>emit('close')},'关闭')])]),
      h('main',{class:'gyc-body'},[h('div',{class:'gyc-grid'},[
        h('section',{class:'gyc-stack'},[
          h('div',{class:'gyc-card'},[h('h3','运行配置'),h('p','常用参数集中在这里；登录与 Token 由状态页自动管理。'),h('div',{class:'gyc-switches'},[
        ...
        h('aside',{class:'gyc-stack'},[
          h('div',{class:'gyc-card'},[h('h3','账号会话'),h('p','登录请在状态页通过扫码或短信完成。'),h('div',{class:'gyc-session'},[line('登录状态',cfg.logged_in?'已登录':'未登录',cfg.logged_in?'gyc-ok':'gyc-warn'),line('插件状态',cfg.enabled?'已启用':'未启用',cfg.enabled?'gyc-ok':'gyc-warn'),line('Client ID',cfg.client_id),line('设备 ID',cfg.device_id)])]),
          h('div',{class:'gyc-card'},[h('h3','使用建议'),h('p','保持默认配置即可满足大多数 MoviePilot 整理场景。'),h('div',{class:'gyc-help'},[h('div',{class:'gyc-tip'},'轮询间隔建议 5–10 秒；过低会增加接口请求频率。'),h('div',{class:'gyc-tip'},'分页大小建议 50–200；目录文件特别多时再提高。'),h('div',{class:'gyc-tip'},'“彻底删除”开启后会跳过回收站，请谨慎使用。'),h('div',{class:'gyc-tip'},'账号授权、Token 刷新与保存全部由插件自动处理。')])])
        ])
